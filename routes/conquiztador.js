...
export default function (io) {
  ...
  ConquiztadorNS.on('connection', (socket) => {
      ...
      if (!/^[a-z0-9]+$/i.test(player.username)) {
        ...
        r = {
          ...
          state: {
            ...
            buzzed: false,
            block: false,
            currentPointsManche2: 0,
            main: null,
            themesList: themesList,
            questionid: null,
            question: null,
            usedBlocks: [],
            finaleQuestions: null
          },
          options: {
            roundTime: 10,
            whitelistEnabled: false,
            whitelist: [],
            couleurs: ['#162d94', '#ffa500'],
            barLength: parseInt(barLength) || 18
          }
        };
        rooms.push(r);
    ...
    socket.on('Conquiz playerData', (player) => {
      ...
      if (!/^[a-z0-9]+$/i.test(player.username)) {
        ConquiztadorNS.in(player.socketId).emit(
          'Conquiz error',
        ...
        if (!r) {
          ...
          if (!r) {
            ...
            if (
              r.players.length >= 2 ||
              (r.options.whitelistEnabled && !r.options.whitelist.includes(player.username))
            ) {
              player.player = false;
    ...
    socket.on('Conquiz estimation', (question) => {
      if (p && p.host) {
        ...
        r.players.forEach((player) => ConquiztadorNS.in(player.socketId).emit('Conquiz estimation', question));
      }
    });
