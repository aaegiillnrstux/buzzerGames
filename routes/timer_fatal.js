  ...
  TFNamespace.on('connection', (socket) => {
    ...
    socket.once('playerDataHost', (player) => {
      ...
      if (!/^[a-z0-9]+$/i.test(player.username)) {
        ...
        r = {
          ...
          spectateurs: [],
          id: player.roomId,
          state: { free: true, start: false, buzzed: false, blocked: false, time_counter: 0, last_question_time: 0 },
          options: { time_start: 60 }
        };
        rooms.push(r);
    ...
    socket.on('playerData', (player) => {
      try {
        ...
        if (!/^[a-z0-9]+$/i.test(player.username)) {
          TFNamespace.in(player.socketId).emit(
            'error',
            "Choississez un pseudo qu'avec des caractères alphanumériques (et sans espaces ! =p)"
          );
    ...
    socket.on('start game', () => {
      try {
        if (p && p.host && !r.state.start) {
          ...
          countdowns[r.id] = setInterval(() => {
            updateCountdown(r, r.players);
          }, 1000);
    ...
    socket.on('restart timer', () => {
      try {
        if (p && p.host && r.state.start && r.state.blocked) {
          ...
          countdowns[r.id] = setInterval(() => {
            updateCountdown(r, players_selected);
          }, 1000);
