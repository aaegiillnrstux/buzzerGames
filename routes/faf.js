...
export default function (io) {
  ...
  var rooms = [
    { players: [], id: 123456789, spectateurs: [], state: { start: false, main: null }, options: { roundTime: 20 } }
  ];
  var listeCodes = [];
    ...
    if (infos.action == 'host') {
      isAdmin(req, res, (isAdminRes) => {
        if (true) {
          ...
          res
            .status(403)
            .render('home', { titre: 'Accès refusé', root: '../../', title: 'Erreur', connected: isAdminRes });
        }
      });
  ...
  router.get('/:code', (req, res) => {
    ...
    if (listeCodes.includes(code) && !room) {
      ...
      isConnected(req, res, (connected, role) => {
        res
          .status(404)
          .render('home', { titre: 'Pas de salles associées', root: '../../', title: 'Erreur', connected: connected });
      });
    }
  });
    ...
    socket.once('FAFplayerDataHost', (player) => {
      ...
      if (!/^[a-z0-9]+$/i.test(player.username)) {
        ...
        r = {
          ...
          options: { roundTime: 20, whitelistEnabled: false, whitelist: [] }
        };
        rooms.push(r);
    ...
    socket.on('FAFplayerData', (player) => {
      try {
        ...
        if (!/^[a-z0-9]+$/i.test(player.username)) {
          fafNamespace
            .in(player.socketId)
            .emit('FAF error', "Choississez un pseudo qu'avec des caractères alphanumériques (et sans espaces ! =p)");
          ...
          if (!r) {
            ...
            r.players.find((play) => {
              return play.username === player.username;
            })
          ) {
            fafNamespace.in(player.socketId).emit('FAF error', 'Player déjà présent dans la room');
    ...
    socket.on('FAF buzzed', (player) => {
      try {
        if (
          ...
          r.state.start &&
          !r.state.buzzed &&
          r.players[r.state.mainInGame].username == player &&
    ...
    socket.on('FAF main', (boite) => {
      try {
        if (p && p.host && r.state.start && r.players[r.state.mainInGame].state == 'free') {
          ...
          r.state.pointsRule[r.state.mainInGame][0] = r.state.pointsRule[r.state.mainInGame][0].filter(
            (item) => item !== boite + 1
          );
          if (r.state.pointsRule[r.state.mainInGame][0].length == 0) {
            ...
            if (r.state.pointsRule[1 - r.state.mainInGame][0].includes(boite)) {
              console.log(
                ` [FAF ${r.id}] ${r.players[r.state.mainInGame].username} a perdu la main car fin de la boite ${boite}`
              );
              r.state.mainInGame = 1 - r.state.mainInGame;
              r.players[r.state.mainInGame].state = 'free';
              r.players[1 - r.state.mainInGame].state = 'blocked';
    ...
    socket.on('FAF change points', (username, points) => {
      try {
        if (p && p.host && points.match(/^-?[0-9]+$/) != null) {
          var player = r.players.find((player) => {
          ...
          if (p.player) {
            if (r) {
              if (r.state.start) {
                ...
                fafNamespace
                  .in(p.roomId)
                  .emit('FAF error', 'Un joueur a quitté la partie, veuillez la relancer et remettre les points');
              }
