...
socket.on('4ALS player init', (room, p) => {
  ...
  room.players.forEach((player) => {
    if (player.host) {
      ...
      $('#player-list').append(
        `<li class="list-group-item" id="${player.username}" >${player.username} <div class="score"><button type="button" id="${player.username}-score" class="btn btn-success score-point edit">${p.points}</button></div></li>`
      );
    }
  });
  currentRoom = room;
...
socket.on('4ALS new player', (player) => {
  $('#player-list').append(
    `<li class="list-group-item" id="${player.username}" >${player.username} <div class="score"><button type="button" id="${player.username}-score" class="btn btn-success score-point edit">${player.points}</button></div></li>`
  );
});
...
function changeCouleurInterieur(n, bool) {
  ...
  } else $(`#interieur-${n}`).css('fill', 'orange');
}
