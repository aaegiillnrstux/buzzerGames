...
const exterieurMatrices = [
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 329.05073, 100.564487)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 327.700571, 26.306825)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 325.900431, -47.950544)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 324.457346, -122.505451)'
];
const interieurMatrices = [
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 329.950785, 100.864505)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 327.700571, 26.606885)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 326.800516, -48.100683)',
  'matrix(0.024755, 0.999694, -1.000075, 0.009348, 325.26359, -122.495312)'
];
const textMatrices = [
  'matrix(2.541135, 0, 0, 1.978724, -341.147898, 151.203053)',
  'matrix(2.541135, 0, 0, 1.978724, -348.952189, 76.573061)',
  'matrix(2.541135, 0, 0, 1.978724, -349.005961, 3.215738)',
  'matrix(2.541135, 0, 0, 1.978724, -351.706218, -73.291895)'
];
const socket = io('/faf');
...
socket.on('FAF new spectateur', (room, spectateur) => {
  ...
  $('#spectateurs-list').append(
    `<li class="list-group-item" id="spectateur-${spectateur.username}">${spectateur.username} <div class="btn-group btn-group-sm" role="group"> <button type="button" id="${spectateur.socketId}-kick" class="btn btn-secondary kick">kick</button> </div> </span></li>`
  );
...
function addPlayer(player, i) {
  ...
  $(`#joueur${i}-score-div`).html(
    `<button type="button" id="${player.username}-score" class="btn btn-success score-point edit" data-bs-toggle="modal" data-bs-target="#modalGivePoints">0</button>`
  );
...
function changeCouleurExterieur(n, bool) {
  ...
  } else $(`#exterieur-${n}`).css('fill', 'darkorange');
}

function updateCountdown() {
  updateColor(boite);
  if (
    $(`#grad-interieur-${boite}-orange`).attr('offset') == '0%' &&
    $(`#grad-interieur-${boite}-blue`).attr('offset') == '0%'
  ) {
    if (boite == 1) {
      boite = 4;
      clearInterval(countdownInterval);
...
function updateColor(n) {
  if (
    $(`#grad-interieur-${n}-orange`).attr('offset') == '100%' &&
    $(`#grad-interieur-${n}-blue`).attr('offset') == '100%'
  ) {
    var newPercent = Math.max(
      0,
      extractNumberFromPercent($(`#grad-interieur-${n}-orange`).attr('offset')) - step / (n * 10)
    );
    $(`#grad-interieur-${n}-orange`).attr('offset', `${newPercent}%`);
    return;
  } else if ($(`#grad-interieur-${n}-blue`).attr('offset') != '0%') {
    var newPercentOrange = Math.max(
      0,
      extractNumberFromPercent($(`#grad-interieur-${n}-orange`).attr('offset')) - step / (n * 10)
    );
    var newPercentBlue = Math.max(
      0,
      extractNumberFromPercent($(`#grad-interieur-${n}-blue`).attr('offset')) - step / (n * 10)
    );
  ...
  if (!start) {
    ...
    if (!main) {
      for (var i = 0; i <= pas; i++) {
        $(`#interieur-${n}`).attr(
          'transform',
          `${interieurMatrices[n - 1]} translate(0,-${(moveX * (pas - i)) / pas}) `
        );
        $(`#exterieur-${n}`).attr(
          'transform',
          `${exterieurMatrices[n - 1]} translate(0,-${(moveX * (pas - i)) / pas})`
        );
