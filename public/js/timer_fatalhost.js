...
function addPlayer(player,i){
    ...
    $(`#player-name-${i}`).html(`<h3 id="joueur-${player.username}">${player.username}</h3> <button type="button" id="${player.socketId}-kick" class="btn btn-secondary kick">kick</button>`);
    $(`#joueur-${player.username}`).data("place",i);
    $(`#player-timer-${i}`).html(`<button type="button" id="${player.username}-timer" class="btn btn-success score-point edit" data-bs-toggle="modal" data-bs-target="#modalGivePoints">${convertTime(player.timer)}</button>`);
