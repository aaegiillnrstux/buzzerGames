...
socket.on('new player', (player, bool) => {
  $('#player-list').append(
    `<li class="list-group-item" id="${player.username}">${player.username} <div class="btn-group btn-group-sm" role="group"> <button type="button" id="${player.socketId}-kick" class="btn btn-secondary kick">kick</button> <button type="button" id="${player.socketId}-block" class="btn btn-warning block">block</button></div><div class="score" style="display: none;"> <button type="button" id="${player.socketId}-score" class="btn btn-success score-point edit" data-bs-toggle="modal" data-bs-target="#modalGivePoints">Edit</button> </div> </span></li>`
  );
