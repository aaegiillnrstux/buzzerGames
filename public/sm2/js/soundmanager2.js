...
(function (window) {
  ...
  function SoundManager(smURL, smID) {
    ...
    var SMSound,
      ...
      _tryInitOnFocus = _isSafari && (typeof _doc.hasFocus === 'undefined' || !_doc.hasFocus()),
    ...
    this.setup = function (options) {
      ...
      if (
        ...
        _needsFlash &&
        _s.ok() &&
        (typeof options.flashVersion !== 'undefined' || typeof options.url !== 'undefined')
      ) {
    ...
    SMSound = function (oOptions) {
              ...
              if (typeof _t.options[stuff] === 'function') {
                ...
                msg.push(
                  ' ' +
                    ...
                    ': {' +
                    sF
                      .substr(sfBracket + 1, Math.min(Math.max(sF.indexOf('\n') - 1, maxLength), maxLength))
                      .replace(/\n/g, '') +
                    '... }'
                );
              } else {
      ...
      this._onconnect = function (bSuccess) {
        bSuccess = bSuccess === 1;
        _s._wD(
          'SMSound._onconnect(): "' + _t.id + '"' + (bSuccess ? ' connected.' : ' failed to connect? - ' + _t.url),
          bSuccess ? 1 : 2
        );
    ...
    */
    ...
    _setVersionInfo = function () {
      ...
      _s.version =
        _s.versionNumber + (_s.html5Only ? ' (HTML5-only mode)' : _fV === 9 ? ' (AS3/Flash 9)' : ' (AS2/Flash 8)');
      ...
      if (_s.useHTML5Audio && _s.hasHTML5) {
        for (item in formats) {
          if (formats.hasOwnProperty(item)) {
            if (
              (formats[item].required && !_s.html5.canPlayType(formats[item].type)) ||
              (_s.preferFlash && (_s.flash[item] || _s.flash[formats[item].type]))
            ) {
    ...
    _createMovie = function (smID, smURL) {
      ...
      function _initMsg() {
        _s._wD(
          '-- SoundManager 2 ' +
            _s.version +
            (!_s.html5Only && _s.useHTML5Audio ? (_s.hasHTML5 ? ' + HTML5 audio' : ', no HTML5 audio support') : '') +
            (!_s.html5Only
              ? (_s.useHighPerformance ? ', high performance mode, ' : ', ') +
                ((_s.flashPollingInterval ? 'custom (' + _s.flashPollingInterval + 'ms)' : 'normal') + ' polling') +
        ...
        if (!_didInit) {
          _s._wD(
            _sm +
              ...
              (p === 0
                ? 'Loading ' + _s.movieURL + ' may have failed (and/or Flash ' + _fV + '+ not present?), '
                : '') +
          ...
          _debugTS(
            'flashtojs',
            false,
            ': Timed out' + _overHTTP ? ' (Check flash security or flash blockers)' : ' (No plugin/missing SWF?)'
          );
        }
        // </d>
              ...
              item +
                ...
                (!_s.html5[item] && _hasFlash && _s.flash[item]
                  ...
                  : _s.preferFlash && _s.flash[item] && _hasFlash
                    ? ' (preferring flash)'
                    : !_s.html5[item]
                      ? ' (' + (_s.audioFormats[item].required ? 'required, ' : '') + 'and no flash support)'
                      : '')
            );
          }
        }
    ...
    _initComplete = function (bNoDisable) {
      ...
      if (_disabled || bNoDisable) {
        if (_s.useFlashBlock && _s.oMC) {
          _s.oMC.className =
            _getSWFCSS() + ' ' + (_s.getMoviePercent() === null ? _swfCSS.swfTimedout : _swfCSS.swfError);
        }
    ...
    _domContentLoaded = function () {
      ...
      if (!_hasFlash && _s.hasHTML5) {
        _s._wD(
          'SoundManager: No Flash detected' + (!_s.useHTML5Audio ? ', enabling HTML5.' : '. Trying HTML5-only mode.')
        );
        _s.setup({
          useHTML5Audio: true,
